...
@property (nonatomic,retain)MessageHomeModel *homeModel;
@property (nonatomic,retain)NSMutableArray *converstionArr;
...
//    self.converstionArr = [NSMutableArray array];
//    for (EMMessage *message in messages)
//    {
//        NSLog(@"-----%@",message.from);
//        [self.converstionArr addObject:message.from];
//    }
//  
//    [self getEMobInfoHttpRequest:[self.converstionArr componentsJoinedByString:@","]];
    ...
    for (EMMessage *message in messages) {
        ...
        if (nickName) {
            ...
            [self getEMobInfoHttpRequest:message.from];
...
-(void)getEMobInfoHttpRequest:(NSString *)em_names
{
    MessageViewModel *viewModel =[MessageViewModel new];
    [viewModel setBlockWithReturnBlock:^(id returnValue) {
        if ([returnValue isKindOfClass:[MessageHomeModel class]]) {
            self.homeModel = (MessageHomeModel *)returnValue;
           
        MessageItem *item = [self.homeModel.items objectAtIndex:0];
        [SCUserProfileEntity saveUserProfileWithUsername:item.em_name forNickName:item.nickname avatarURLPath:item.avatar];
        }
    } WithFailureBlock:^{
    ...
    [params setObject:em_names forKey:@"em_names"];
    
    [viewModel getEMobInfoTaskWithParams:params];
}
