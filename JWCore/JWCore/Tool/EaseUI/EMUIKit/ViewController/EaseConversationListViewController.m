...
- (void)viewDidLoad {
    ...
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(tableViewDidTriggerHeaderRefresh) name:KUSER_RELOAD_MESSAGE object:nil];
}
...
- (void)tableViewDidTriggerHeaderRefresh
{
    if ([SysParams isLogon]) {
        ...
        [self getEMobInfoHttpRequest];
    }
    else
    {
        [self conversationTableViewReload];
 
    }
}

#pragma mark - info request
-(void)getEMobInfoHttpRequest
{
    ...
    [SCUserProfileEntity saveUserProfileWithUsername:name forNickName:[[[[SysParams sharedInstance]logonModel] homeModel] nickname] avatarURLPath:[[[[SysParams sharedInstance]logonModel] homeModel] avatar]];
    for (EMConversation *converstion in conversations)
    {
        NSLog(@"-----%@",converstion.conversationId);
        [self.converstionArr addObject:converstion.conversationId];
    }
    ...
    [viewModel setBlockWithReturnBlock:^(id returnValue) {
        ...
        if ([returnValue isKindOfClass:[MessageHomeModel class]]) {
            ...
            for (int i =0;i<self.converstionArr.count; i++ ) {
                MessageItem *item = [self.homeModel.items objectAtIndex:i];
    ...
    [params setObject:[self.converstionArr componentsJoinedByString:@","] forKey:@"em_names"];
    
    [viewModel getEMobInfoTaskWithParams:params];
}

-(void)conversationTableViewReload
{
    NSArray *conversations = [[EMClient sharedClient].chatManager getAllConversations];
    ...
    for (EMConversation *converstion in sorted) {
        ...
        if ([SysParams isLogon]){
            NSString *nickName = [SCUserProfileEntity getNickNameWithUsername:converstion.conversationId];
            if (nickName) {
            ...
            //        }
            NSString *avatarURLPath = [SCUserProfileEntity getavatarURLPathWithUsername:converstion.conversationId];
            if (avatarURLPath) {
